...
public sealed class ObservabilityMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        RecordActiveRequests(context, activeRequests);
        ...
        finally
        {
            ...
            RecordRequestDuration(context, stopwatch);

            activeRequests = Interlocked.Decrement(ref _activeRequests);

            RecordActiveRequests(context, activeRequests);
            RecordTrace(context, activity, stopwatch);
    ...
    private void RecordRequest(HttpContext context)
    {
    ...
    private void RecordRequestDuration(HttpContext context, Stopwatch stopwatch)
    {
        _metric.Record(new MetricEntry
        {
            Timestamp = DateTimeOffset.UtcNow,
    ...
    private void RecordActiveRequests(HttpContext context, long activeRequests)
    {
        _metric.Record(new MetricEntry
        {
            Timestamp = DateTimeOffset.UtcNow,
    ...
    private void RecordError(HttpContext context)
    {
        _metric.Record(new MetricEntry
        {
            Timestamp = DateTimeOffset.UtcNow,
    ...
    private void RecordTrace(HttpContext context, Activity activity, Stopwatch stopwatch)
    {
